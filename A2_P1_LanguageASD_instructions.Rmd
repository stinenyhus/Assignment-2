...
clean %>% subset(VISIT == "1") %>% group_by(Diagnosis, Gender) %>% summarise(SUBJ = n())

clean %>% subset(VISIT == "1") %>% group_by(Diagnosis, Ethnicity) %>% summarise(SUBJ = n())
```
...
ggplot(clean, aes(VISIT, CHI_MLU, group = Diagnosis, fill = Diagnosis))+
  geom_point()+
...
model1 <- lmer(CHI_MLU ~ VISIT*Diagnosis + (1|SUBJ) + (0+VISIT|SUBJ), clean)
summary(model1)
```
